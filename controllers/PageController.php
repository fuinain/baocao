...
class PageController extends Controller
{
    ...
    public function handleCallback(Request $request)
    {
           $id= $_GET['vnp_TxnRef'];
          $gia=  $_GET['vnp_Amount'];
          $type=  $_GET['vnp_CardType'];
        
        if (isset($_GET['vnp_Amount'])) {
            // Assuming Application::$app->db->lastInsertId() returns the last inserted ID
            $idOrder = Application::$app->db->lastInsertId();
            foreach ($cart as $value) {
                $orders['paymentType'] = $value->type;
                $orders['price'] = $value->gia;  // Assuming $value has a 'so_luong_mua' property
                $orders['id'] = $idOrder;
                
                // Assuming $classModelOrder is an instance of the order model
                $classModelOrder->loadData($data);
                $classModelOrder->save();
            }
   ...
   public function vnpayment(Request $request) {
    ...
    $vnp_Returnurl = "http://localhost:8888/payment/callback";
    $vnp_TmnCode = "7GE86HIK"; // Mã website tại VNPAY
    $vnp_HashSecret = "LPGSWWHIBGONFJXQVIOXZRISMIKHTCSM"; // Chuỗi bí mật
   
    $vnp_TxnRef = time() . mt_rand(10000, 99999);
